...
当前buffer设计是基于单工模式的，即：要么读，要么写.不会存在同时读写的情况。只有数据被操作完成（读完或者写完）后state才能被改变，同时可能要改变owner。
## 一、bufer 操作基本场景
对buffer的操作分为以下三个场景
...
#### 第一个场景 从 channel 向 proxybuffer 写入数据。
    ...
    4. proxybuffer 压缩。
       每次开始向proxybuffer 写入数据前，判断当前proxybuffer 容量是否大于总容量的1/3（writeIndex > buffer.capacity() * 1 / 3).
...
#### 第三个场景 channel 从 buffer 中读取数据。
    ...
    5. proxybuffer 压缩。
       每次从proxybuffer读取数据写入到channel前，判断当前proxybuffer 容量是否大于总容量的2/3（writeIndex > buffer.capacity() * 1 / 3).
